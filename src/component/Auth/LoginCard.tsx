...
const LoginCard = ({getEmail}: {getEmail : (arg0: string) => void})=> {
  const [email, setEmail] = useState("");
  const { setLoginWithCard, setShowLogin, setShowData, setShowOTP } = store();
  ...
  const login = async () => {
    if (email === "") {
      ...
      try {
        ...
        if (response.message.includes("OTP")) { //If user is not verified
          getEmail(email);
          setShowOTP(true);
        } else {

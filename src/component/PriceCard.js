  ...
  return (
      <div className={` basis-1/3 p-8 border border-[#E3E3E2] ${color === "green" ? 'bg-[#061F13] text-white' : 'bg-[#D5DDDC] text-[#2E2E27]'}`}>
          ...
          <div className={`pt-8 border-t-[0.5px] border-[#D5DDDC] flex flex-col gap-6`}>
            {benefits.map((benefit, i) => <p key={i} className={`pl-10 bg-left bg-[length:24px] bg-no-repeat text-[16px] leading-6 md:text-[20px] md:leading-7 ${color === "green" ? 'bg-white-tick': 'bg-green-tick'}`}>{benefit}</p>)}
          </div>
      </div>
  )
}
...
export const SupportCard = () => {
    ...
    return (
        <div className={`basis-1/3 p-8 border border-[#E3E3E2] bg-[#FFFFFF] text-[#2E2E27] text-center flex flex-col justify-center items-center`}>
          ...
          <p className='text-[#64645F] text-[16px] leading-6 md:text-[20px] md:leading-7'>Learn more about how pptdesigner works and how it can help you.</p>
